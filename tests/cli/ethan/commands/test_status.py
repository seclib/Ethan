...
class TestStatusCommand:
    ...
    def test_status_online(self, capsys) -> None:
        ...
        with mock.patch("cli.commands.status.alive", return_value=True):
            with mock.patch(
                "cli.commands.status.get_state",
                return_value={"mode": "running", "active_goal": "test", "running_tasks": 2},
            ):
                result = cmd_status([])
    ...
    def test_status_partial_state(self, capsys) -> None:
        ...
        with mock.patch("cli.commands.status.alive", return_value=True):
            with mock.patch("cli.commands.status.get_state", return_value={"mode": "running"}):
                cmd_status([])
                captured = capsys.readouterr()
                assert "ONLINE" in captured.out
