...
class TestSuggestCommand:
    ...
    def test_suggest_default(self, capsys) -> None:
        ...
        with mock.patch("cli.commands.suggest.mem.recent", return_value=[{"text": "recent cmd"}]):
            with mock.patch(
                "cli.commands.suggest.mem.frequent",
                return_value=[{"text": "frequent cmd", "count": 5}],
            ):
                result = cmd_suggest([])
    ...
    def test_suggest_prefix_match(self, capsys) -> None:
        ...
        with mock.patch(
            "cli.commands.suggest.mem.suggest_prefix", return_value=["hello world", "help"]
        ):
