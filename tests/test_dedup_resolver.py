...
def _file_item(file_id, name, **kw):
    return ScannedItem(
        ...
        collection_ids=kw.get("collection_ids", []),
        folder_ids=kw.get("folder_ids", []),
        rag_document_id=kw.get("rag_document_id", ""),
...
def test_delete_after_confirm_with_flag_removes_and_deletes():
    async def scenario():
        ...
        f2 = _file_item(
            "f2", "a.txt", project_ids=["p2"], collection_ids=["c2"], rag_document_id="r2"
        )
        group = _group(f1, f2)
...
def test_repair_reference_prefers_file():
    async def scenario():
        ...
        group = DuplicateGroup(
            group_id="g1", category=DuplicateCategory.BROKEN_REFERENCE, items=[rag, f1]
        )
