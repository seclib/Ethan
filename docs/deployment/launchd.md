...
Ethan includes a launchd property list (plist) for running the API server as a background service on macOS. This provides automatic startup at login, automatic restart if the process exits, and log capture.

## Prerequisites

Before installing the service, ensure that Ethan is installed and the `jarvis` command is available at `/usr/local/bin/jarvis`. If you installed via `uv` or `pip` with a different prefix, adjust the path in the plist accordingly.

```bash
git clone https://github.com/open-jarvis/Ethan.git && cd Ethan && uv sync --extra server
...
```bash
cp deploy/launchd/com.ethan.plist ~/Library/LaunchAgents/
launchctl load ~/Library/LaunchAgents/com.ethan.plist
```
...
```bash
launchctl list | grep ethan
```

You should see a line with the PID and the label `com.ethan`. A `0` in the status column indicates the service is running normally.
...
The provided plist file at `deploy/launchd/com.ethan.plist`:
...
<dict>
    ...
    <string>/tmp/ethan.stdout.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/ethan.stderr.log</string>
</dict>
</plist>
```
...
|----------------------|--------------------------------|------------------------------------------------------------------------------------------------------|
| `Label`              | `com.ethan`               | Unique identifier for the service. Used with `launchctl` commands to manage the service.             |
...
| `StandardOutPath`    | `/tmp/ethan.stdout.log`   | File where standard output is written. Contains server startup messages and access logs.             |
| `StandardErrorPath`  | `/tmp/ethan.stderr.log`   | File where standard error is written. Contains error messages and stack traces.                      |
...
cat /tmp/ethan.stdout.log

# View standard error (errors, warnings)
cat /tmp/ethan.stderr.log

# Follow logs in real time
tail -f /tmp/ethan.stdout.log /tmp/ethan.stderr.log
```
...
!!! tip "Persistent log location"
    ...
    ```xml
    <key>StandardOutPath</key>
    <string>/Users/yourname/.ethan/ethan.stdout.log</string>
    <key>StandardErrorPath</key>
    <string>/Users/yourname/.ethan/ethan.stderr.log</string>
    ```
...
launchctl unload ~/Library/LaunchAgents/com.ethan.plist
```
...
```bash
# Stop the service
launchctl stop com.ethan

# Start the service
launchctl start com.ethan
```
...
```bash
# List all loaded services matching "ethan"
launchctl list | grep ethan
```
...
| Label  | The service label (`com.ethan`)                           |
...
The instructions above install the service as a **user agent** (runs only when you are logged in). To run Ethan as a system-wide daemon that starts at boot regardless of user login:
...
```bash
sudo cp deploy/launchd/com.ethan.plist /Library/LaunchDaemons/
sudo chown root:wheel /Library/LaunchDaemons/com.ethan.plist
sudo launchctl load /Library/LaunchDaemons/com.ethan.plist
```
