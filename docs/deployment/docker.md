...
Ethan provides Docker images for both CPU-only and GPU-accelerated deployments, along with a Docker Compose configuration that bundles the API server with an Ollama inference backend.
...
1. **Builder stage** -- installs `uv` and the `ethan[server]` package (which includes FastAPI, uvicorn, and all server dependencies) from the project source.
...
```bash
docker build -t ethan:latest .
```

Run it standalone:

```bash
docker run -d -p 8000:8000 ethan:latest
```
...
```bash
docker build -f Dockerfile.gpu -t ethan:gpu .
```
...
```bash
docker run -d --gpus all -p 8000:8000 ethan:gpu
```
...
The `docker-compose.yml` defines a complete deployment with the Ethan API server and an Ollama backend:
...
To use a custom `config.toml`, mount it into the container at the expected path (`~/.ethan/config.toml`, which is `/root/.ethan/config.toml` in the container):
...
services:
  jarvis:
    ...
    ports:
      - "8000:8000"
    volumes:
      - ./my-config.toml:/root/.ethan/config.toml:ro
    environment:
      - OPENJARVIS_ENGINE_DEFAULT=ollama
      - OPENJARVIS_OLLAMA_HOST=http://ollama:11434
    depends_on:
...
To persist telemetry data, memory databases, and trace records across container restarts, mount the entire Ethan data directory:
...
```bash
docker run -d -p 9000:9000 ethan:latest \
