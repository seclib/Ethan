...
!!! note "Optional component"
    The scheduler is a standalone module (`ethan.scheduler`). It is not wired into the default `Jarvis` / `JarvisSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `jarvis scheduler start`.
...
# Set up storage
store = SchedulerStore(db_path="~/.ethan/scheduler.db")  # (1)!
...
Scheduler settings live in the `[scheduler]` section of `~/.ethan/config.toml`.

```toml title="~/.ethan/config.toml"
[scheduler]
enabled = false
db_path = "~/.ethan/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.ethan/scheduler.db`     | SQLite database path                       |
