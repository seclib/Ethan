...
!!! note "Backward compatibility"
    The registry alias `"react"` maps to `NativeReActAgent`. The old import `from ethan.agents.react import ReActAgent` also still works.
...
!!! warning "Requirements"
    Requires Node.js 22+ on `PATH` and an `ANTHROPIC_API_KEY` environment variable (or pass `api_key=` directly). The bundled runner is auto-installed to `~/.ethan/claude_code_runner/` on first use via `npm install`.

**How it works:**

1. On first call, copies the bundled `claude_code_runner/` to `~/.ethan/claude_code_runner/` and runs `npm install --production` if `node_modules` is missing.
...
**When to use:** For software engineering tasks where the Claude Agent SDK's built-in tools (code editing, bash execution, file operations) provide capabilities beyond what Ethan tool-calling agents support.

```python
from ethan.agents.claude_code import ClaudeCodeAgent
...
!!! info "accepts_tools = False"
    `ClaudeCodeAgent` does not accept Ethan tools via `--tools`. Tool access for the Claude agent is configured separately via the `allowed_tools` constructor parameter, which passes tool names understood by the Claude Agent SDK itself.

---

## OpenCodeAgent

The `OpenCodeAgent` delegates coding tasks to [opencode](https://opencode.ai), the open-source coding agent, running it **on your local engine**. opencode handles the agentic loop, file edits, and tool use; Ethan supplies the model — keeping coding-agent work local-first.

!!! warning "Requirements"
    Requires the `opencode` binary on `PATH` (`npm i -g opencode-ai` or `brew install anomalyco/tap/opencode`). It is **not** bundled; `run()` returns a clear error if it is missing. No `ANTHROPIC_API_KEY` needed — inference goes through your Ethan engine.

**How it works:**

1. Derives an OpenAI-compatible base URL from the `engine` (e.g. Ollama/vLLM/llama.cpp at `<host>/v1`) and writes an `opencode.json` in the workspace registering it as an `@ai-sdk/openai-compatible` provider (`ethan/<model>`).
...
| `provider_id`       | `str`             | `"ethan"`   | opencode provider id to register/use                     |
...
```python
from ethan.agents.opencode import OpenCodeAgent
...
```python
from ethan.agents.operative import OperativeAgent
...
```python
from ethan.agents.monitor_operative import MonitorOperativeAgent
...
```python
from ethan.sandbox import ContainerRunner, SandboxedAgent
from ethan.agents.simple import SimpleAgent

runner = ContainerRunner(
    image="ethan-sandbox:latest",
    timeout=60,
    mount_allowlist_path="/etc/ethan/mount_allowlist.json",
)
...
| Parameter              | Type   | Default                      | Description                                    |
|------------------------|--------|------------------------------|------------------------------------------------|
| `image`                | `str`  | `"ethan-sandbox:latest"`| Docker image to run                            |
