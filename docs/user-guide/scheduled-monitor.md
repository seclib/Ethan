...
This writes a pre-configured `~/.ethan/config.toml` for the operative agent with scheduling support.
...
- **All data stays local**: State is stored in `~/.ethan/` using the configured memory backend. Nothing leaves your machine.
