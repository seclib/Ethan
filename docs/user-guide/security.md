...
Ethan includes a security layer that scans prompts and model outputs for secrets, personally identifiable information (PII), and sensitive file paths. The system is designed to be composable: scanners run as a pipeline, and the `GuardrailsEngine` wrapper drops in front of any inference backend without changing how the rest of your code works.
...
=== "Warn mode (default)"
    ...
    from ethan.security.guardrails import GuardrailsEngine
    from ethan.security.types import RedactionMode
...
from ethan.security.scanner import PIIScanner
...
from ethan.security.file_policy import is_sensitive_file, filter_sensitive_paths
...
audit = AuditLogger(db_path="~/.ethan/audit.db", bus=bus)
...
import time
from ethan.security.audit import AuditLogger
from ethan.security.types import SecurityEvent, SecurityEventType
...
audit = AuditLogger(db_path="~/.ethan/audit.db")
...
Security settings live in the `[security]` section of `~/.ethan/config.toml`.
...
| `audit_log_path` | `str` | `~/.ethan/audit.db` | Path to the SQLite audit log |
...
from ethan.security.types import ScanFinding, ScanResult, ThreatLevel
...
- [API Reference: Security](../api-reference/ethan/security/index.md) — full class and function signatures
