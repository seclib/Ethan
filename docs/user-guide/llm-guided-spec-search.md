...
```bash
python examples/ethan/spec_search_quickstart.py
```
...
`ethan.learning.spec_search.composite_reward.score_batch`.
...
`configs/ethan/examples/spec-search-quickstart.toml`. Copy it to
`~/.ethan/config.toml` (or set `OPENJARVIS_CONFIG` to it) and the
...
| `trace_store` | `MagicMock` | `ethan.traces.store.TraceStore(home / "traces.db")` |
| `student_runner` | `MagicMock` | `ethan.learning.spec_search.student_runner.VLLMStudentRunner(host=..., model=...)` |
| `judge` | `MagicMock` | `ethan.evals.core.scorer.LLMJudgeScorer(...)` (or a deterministic scorer if your benchmark provides one) |
| `session_store` | `MagicMock` | `ethan.learning.spec_search.storage.session_store.SessionStore(home / "learning" / "sessions.db")` |
| `checkpoint_store` | `MagicMock` | `ethan.learning.spec_search.checkpoint.store.CheckpointStore(home / "learning" / "checkpoints")` |
...
1. Create `src/ethan/evals/datasets/<corpus>.py` implementing
   `DatasetProvider` (`adp.py` is a small reference). The provider's
   `load(max_samples, seed, split)` must respect `split` via
   `apply_split` from `ethan.evals.core.splits`.
2. Register: `@DatasetRegistry.register("<corpus>")`.
3. Feed it to the proposer via the trace store:

```python
from ethan.evals.datasets.adp import ADPDataset
from ethan.learning.spec_search.external_adapter import (
...
store = TraceStore("~/.ethan/traces.db")
...
`src/ethan/evals/backends/jarvis_agent.py` previously hardcoded
`builder.telemetry(telemetry).traces(True).build()`, ignoring the
`telemetry` parameter. This silently caused every agent-backend
evaluation to write to `~/.ethan/traces.db` regardless of caller
...
- `examples/ethan/spec_search_quickstart.py` — runnable end-to-end demo.
- `configs/ethan/examples/spec-search-quickstart.toml` — prebuilt config.
- `src/ethan/learning/spec_search/orchestrator.py` — `SpecSearchOrchestrator` (single session).
- `src/ethan/learning/spec_search/multi_session.py` — `SpecSearchLoop` (Algorithm 1).
- `src/ethan/learning/spec_search/composite_reward.py` — paper Eq. 1.
- `src/ethan/learning/spec_search/gate/benchmark_gate.py` — `GateOK` predicate.
- `src/ethan/learning/spec_search/external_adapter.py` — corpus → trace adapter.
