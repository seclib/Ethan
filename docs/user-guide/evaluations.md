...
The Ethan evaluation framework (`ethan.evals`) measures model **correctness and accuracy** on academic datasets. It ships inside the main `ethan` package (at `src/ethan/evals/`) and is designed specifically for research workflows where you need reproducible, dataset-driven quality assessments.

!!! info "Evals vs. Benchmarks"
    Ethan has two distinct measurement systems that complement each other:

    | System | Module | Measures | Entry Point |
    |--------|--------|----------|-------------|
    | **Evaluations** | `ethan.evals` | Correctness on academic datasets (accuracy, pass rate) | `jarvis eval` |
    | **Benchmarks** | `ethan.bench` | Engine performance (latency, throughput) | `jarvis bench` |
...
The evaluation framework is part of the main `ethan` package — no separate install or extra is required. The standard dev setup is enough:

```bash
uv sync --extra dev
```

The framework's core dependencies (`click`, `datasets`, `rich`) are base dependencies of `ethan`. Two optional extras enable experiment tracking integrations:
...
!!! note "Python version requirement"
    Python 3.10 requires the `tomli` package for TOML config parsing. `ethan` declares it as a conditional dependency, so it is installed automatically.
...
| `python -m ethan.evals {list,run,run-all,summarize,reparse-judge}` | Full research surface, including judge configuration, the agentic runner, and episode mode. |

The `ethan-eval` console script is an alias for `python -m ethan.evals` — same commands, same options. This guide uses `jarvis eval` wherever its option set suffices and the module form for research-only options.

---

## Datasets

The framework ships with **40 registered benchmarks** covering academic reasoning, agentic tasks, coding, retrieval, conversation quality, and practical use-case benchmarks. Datasets are grouped by category below; `uv run python -m ethan.evals list` prints the authoritative registry.

### Use-Case Benchmarks

These benchmarks evaluate models on practical tasks that mirror real Ethan use cases.
...
```bash
uv run jarvis eval run --config src/ethan/evals/configs/use_case_v2_cloud.toml
```
...
```bash
uv run jarvis eval run --config src/ethan/evals/configs/use_case_v2_local.toml
```
...
```bash
uv run python -m ethan.evals list
```
...
#### Research-only options (`python -m ethan.evals run`)
...
```bash
uv run python -m ethan.evals run-all -m qwen3:8b

# With options
uv run python -m ethan.evals run-all -m gpt-5-mini -n 100 --output-dir results/gpt5mini/
```
...
```bash
uv run python -m ethan.evals summarize results/supergpqa_qwen3-8b.jsonl
```
...
!!! tip "Engine-level alternative for vLLM"
    ...
    ```bash
    VLLM_HOST=http://gpu-node:8000 uv run python -m ethan.evals run \
...
```bash
uv run jarvis eval run --config src/ethan/evals/configs/full-suite.toml
```
...
```toml title="src/ethan/evals/configs/full-suite.toml"
...
```toml title="src/ethan/evals/configs/minimal.toml"
[[models]]
name = "qwen3:8b"
...
```toml title="src/ethan/evals/configs/single-run.toml"
[meta]
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | str | required | Any registered benchmark key (see `uv run python -m ethan.evals list`) |
...
uv run python -m ethan.evals run -b supergpqa -m qwen3:8b -w 8 -n 500
```
...
- [Python SDK](python-sdk.md) — Programmatic access to Ethan inference and agents
