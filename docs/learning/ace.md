...
Ethan supports [ACE](https://github.com/ace-agent/ace) as a third
...
ACE is **not on PyPI** as of Ethan v1.0.1, and the upstream
...
```bash
# 1. Clone ACE somewhere outside your Ethan checkout
git clone https://github.com/ace-agent/ace.git ~/code/ace
cd ~/code/ace
curl -LsSf https://astral.sh/uv/install.sh | sh   # if you don't have uv
uv sync

# 2. Make ACE's src/ importable from your Ethan venv
...
# 4. Verify the import resolves from Ethan's venv
python -c "from ethan.learning.agents.ace_optimizer import HAS_ACE; print(HAS_ACE)"
# True
```

If `HAS_ACE` prints `False`, the `.pth` file isn't being picked up —
verify the path matches `site.getsitepackages()[0]` for the same
Python interpreter you're using to run Ethan.

## Configuration

ACE is configured under `[learning.agent.ace]` in your Ethan
config TOML:
...
task_name = "ethan"
save_dir = ""               # default: ~/.ethan/learning/ace/<task>/
...
The Ethan runtime will pick up the playbook on next agent start
(via the same sidecar overlay mechanism the Skills System uses).

## Trace adapter behavior

Ethan traces are adapted into ACE's `train_samples` /
...
in `src/ethan/learning/agents/ace_optimizer.py` — it does a
...
- **Single provider per run.** ACE assigns the same `api_provider` to
  all three roles. To mix providers, run ACE outside Ethan and
