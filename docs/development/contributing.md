...
Ethan uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=ethan --cov-report=html
```
...
Ethan uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/ethan/`:
...
try:
    import ethan.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
