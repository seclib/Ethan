...
through Pearl's vLLM miner and Ethan.

Ethan can point `vllm-pearl` at a model id, but a raw Hugging Face model is
...
Ethan only supports Pearl models published by the `pearl-ai` Hugging Face
organization. Private staging artifacts and Ethan-specific conversion
...
`jarvis mine start`, vLLM `/v1/models`, Ethan inference routing, Pearl
...
org artifacts. They remain `planned` in Ethan until we have clean
...
2. Convert the target model.
   ...
   Ethan includes an experimental local converter for this work:
