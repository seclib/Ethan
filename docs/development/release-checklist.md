...
8. **`jarvis-uninstall`** removes `~/.ethan/` and `~/.local/bin/jarvis*`. Verify with `ls`.
...
- Delete `~/.ethan/config.toml` — verify bare `jarvis` re-runs init.
- Delete `~/.ethan/.venv` — verify re-running curl heals it.
