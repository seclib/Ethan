...
prove Ethan wiring; this validates that a real H100/H200 host can mine
through Pearl and serve inference through Ethan.
...
- Docker container `ethan-pearl-miner` starts.
- `~/.ethan/runtime/mining.json` is written.
...
```bash
docker ps --filter name=ethan-pearl-miner
cat ~/.ethan/runtime/mining.json
...
## Verify Ethan Inference Uses Mining Endpoint
...
- If metric names differ, attach `/tmp/pearl-gateway-metrics.txt` to the PR and
  update `src/ethan/mining/_metrics.py`.
...
docker ps --filter name=ethan-pearl-miner
test ! -e ~/.ethan/runtime/mining.json
```
...
- Ethan inference succeeds through `vllm-pearl-mining`.
...
docker inspect ethan-pearl-miner || true
