...
!!! note "Before you begin"
    This tutorial assumes Ethan is installed with Ollama running and a model available (e.g., `qwen3.5:9b`). If you have not completed setup yet, start with the [Quick Start guide](../getting-started/quickstart.md).

## Step 1: Install Skills from Hermes Agent

Ethan can import skills from the [Hermes Agent](https://github.com/NousResearch/hermes-agent) skill library maintained by NousResearch. Let's install a few useful ones.
...
The first install clones the Hermes repo to `~/.ethan/skill-cache/hermes/` (one-time, ~5s). Subsequent installs reuse the cache.
...
```bash
cat ~/.ethan/skills/hermes/arxiv/SKILL.md | head -40
```

The `.source` file records provenance:

```bash
cat ~/.ethan/skills/hermes/arxiv/.source
```
...
```bash
mkdir -p ~/.ethan/skills/my-reviewer
```

Write a SKILL.md:

```bash
cat > ~/.ethan/skills/my-reviewer/SKILL.md << 'EOF'
---
name: my-reviewer
...
Each query produces a trace in `~/.ethan/traces.db` with skill metadata tags (`skill`, `skill_source`, `skill_kind`).
...
Discovered skills land in `~/.ethan/skills/discovered/` and automatically appear in `jarvis skill list` on the next session.
...
This produces overlay files at `~/.ethan/learning/skills/<skill-name>/optimized.toml` with improved descriptions and few-shot examples extracted from your best traces.
...
For a hands-off experience, add this to `~/.ethan/config.toml`:
