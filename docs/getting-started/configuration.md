---
title: Configuration
description: Complete reference for Ethan configuration
---

# Configuration

Ethan uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
Ethan creates the `~/.ethan/` directory and populates it with a default config when you run `jarvis init`.
...
3. Writes `~/.ethan/config.toml` with sensible defaults
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, Ethan automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, Ethan uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
| `db_path` | string | `~/.ethan/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
[traces]
enabled = false
db_path = "~/.ethan/traces.db"
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.ethan/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.ethan/skills/` | Directory where skills are installed. |
...
overlay_dir = "~/.ethan/learning/skills/"
```
...
| `overlay_dir` | string | `~/.ethan/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `jarvis init`, Ethan probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.ethan/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure Ethan entirely from Python without a TOML file:
...
Ethan respects the following environment variables:
