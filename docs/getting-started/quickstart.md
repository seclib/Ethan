---
title: Quick Start
description: Get up and running with Ethan in minutes
search:
  boost: 3
---
...
Ethan is a modular AI assistant framework. Here's what developers build with it:
...
=== "Index Docs & Ask"
    ...
    !!! warning "Requires the Rust extension"
        `jarvis memory index` and `jarvis memory search` import `ethan_rust`. If you
        skipped the `uv run maturin develop -m rust/crates/ethan-python/Cargo.toml`
        step in [Installation](installation.md), these commands fail with
        `ModuleNotFoundError: No module named 'ethan_rust'`. Build the extension
...
=== "Morning Digest"
    ...
    ```bash
    cp configs/ethan/examples/morning-digest-mac.toml ~/.ethan/config.toml
...
Copy one of these to `~/.ethan/config.toml` to get a pre-configured setup:

| Config | For | What it does |
|--------|-----|-------------|
| [`chat-simple.toml`](https://github.com/open-jarvis/Ethan/blob/main/configs/ethan/examples/chat-simple.toml) | Any machine | Lightweight chat, no tools -- simplest setup |
| [`code-assistant.toml`](https://github.com/open-jarvis/Ethan/blob/main/configs/ethan/examples/code-assistant.toml) | Any machine | Orchestrator agent with code execution, file I/O, shell |
| [`deep-research.toml`](https://github.com/open-jarvis/Ethan/blob/main/configs/ethan/examples/deep-research.toml) | Any machine | Multi-hop research across indexed documents with citations |
| [`scheduled-monitor.toml`](https://github.com/open-jarvis/Ethan/blob/main/configs/ethan/examples/scheduled-monitor.toml) | Any machine | Persistent operative agent on a cron schedule |
| [`morning-digest-mac.toml`](https://github.com/open-jarvis/Ethan/blob/main/configs/ethan/examples/morning-digest-mac.toml) | Mac (Apple Silicon) | Daily spoken briefing from email, calendar, health, news |
| [`morning-digest-linux.toml`](https://github.com/open-jarvis/Ethan/blob/main/configs/ethan/examples/morning-digest-linux.toml) | Linux / GPU server | Same, with vLLM support |
| [`morning-digest-minimal.toml`](https://github.com/open-jarvis/Ethan/blob/main/configs/ethan/examples/morning-digest-minimal.toml) | Any machine | Just Gmail + Calendar |
...
This guide walks through the core workflows of Ethan: the browser app, CLI, Python SDK, agents with tools, memory, benchmarks, and the API server.

!!! info "Prerequisites"
    Make sure you have [installed Ethan](installation.md) and have at least one inference backend running (e.g., `ollama serve`).

## Browser App

The quickest way to experience Ethan is the full chat UI running in your browser:

```bash
git clone https://github.com/open-jarvis/Ethan.git
cd Ethan
...
This runs hardware auto-detection (GPU vendor, VRAM, CPU, RAM) and writes a config file to `~/.ethan/config.toml` with sensible defaults for your system. It also selects the recommended inference engine.
...
The simplest way to interact with Ethan is the `ask` command:

```bash
jarvis ask "What is the capital of France?"
```

Ethan will auto-detect a running engine, select a model using the configured router policy, and return the response.
...
```python
# Use default config (auto-detected hardware, ~/.ethan/config.toml)
j = Jarvis()
...
Index a file or directory. Ethan chunks the content and stores it in the configured memory backend (SQLite/FTS5 by default).
...
When you have indexed documents, Ethan automatically injects relevant context into your queries. The memory system searches for chunks matching your query and prepends them as system context before sending to the model.
...
Ethan provides an OpenAI-compatible API server for integration with existing tools and frontends.
...
Ethan records telemetry for every inference call (timing, tokens, cost). View aggregated statistics:
