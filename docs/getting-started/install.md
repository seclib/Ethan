...
|---|---|---|
| **macOS** | `curl -fsSL https://open-jarvis.github.io/Ethan/install.sh \| bash` | [macOS install](macos.md) |
| **Linux** | `curl -fsSL https://open-jarvis.github.io/Ethan/install.sh \| bash` | [Linux install](linux.md) |
| **WSL2 on Windows** | `curl -fsSL https://open-jarvis.github.io/Ethan/install.sh \| bash` (run inside Ubuntu) | [WSL2 install](wsl2.md) |
| **Native Windows** | `irm https://open-jarvis.github.io/Ethan/install.ps1 \| iex` | [Native Windows install](windows-native.md) |
| **Desktop GUI** | Download from the [latest release](https://github.com/open-jarvis/Ethan/releases) | — |
...
```bash
curl -fsSL https://open-jarvis.github.io/Ethan/install.sh | bash
```
...
!!! info "Install URL"
    ...
    so HTTPS always works. You may also see `https://ethan.ai/install.sh`
    referenced in older docs — that domain is community-operated and has had
    intermittent TLS issues ([#337](https://github.com/open-jarvis/Ethan/issues/337)).
...
| Foreground | Clone Ethan repo | `~/.ethan/src/` |
| Foreground | Create Python 3.11 venv | `~/.ethan/.venv/` |
...
| Foreground | Write `config.toml` (auto-detected hardware + engine + model) | `~/.ethan/config.toml` |
...
Re-running the curl line is safe. The installer reads `~/.ethan/.state/install-state.json` and skips completed steps. If your venv got nuked, re-running heals it.
...
|---|---|---|
| `OPENJARVIS_HOME` | `$HOME/.ethan` | Install location. |
| `OPENJARVIS_REPO_URL` | `https://github.com/open-jarvis/Ethan.git` | Source repo for the clone step. |
...
Removes `~/.ethan/`, `~/.local/bin/jarvis`, and `~/.local/bin/jarvis-uninstall`. Leaves Ollama, uv, and the Rust toolchain in place (they may be used by other tools); the script prints removal hints.
...
```bash
~/.ethan/.scripts/install-rust.sh && ~/.ethan/.scripts/build-extension.sh
```
...
```bash
jarvis doctor
~/.ethan/.scripts/pull-model.sh qwen3.5:9b
```
