...
Ethan ships **anonymous usage telemetry** by default so the team can
...
- **Self-hosted backend** on the Ethan team's PostHog instance —
...
[`src/ethan/analytics/events.py`](../src/ethan/analytics/events.py).
...
1. [`src/ethan/analytics/redaction.py`](../src/ethan/analytics/redaction.py) — value-level pattern matching (20+ regexes for PII).
2. [`src/ethan/analytics/events.py`](../src/ethan/analytics/events.py) — structural allowlist (event name + property name + type validator).
...
- **Production target**: A self-hosted PostHog instance at
  `analytics.ethan.ai`, Hetzner US-East. Single-tenant, operated
  by the Ethan team.
- **Never** sold, shared with advertisers, or used for anything other
  than improving Ethan.
...
- Local telemetry (FLOPs, energy, latency stored in
  `~/.ethan/telemetry.db`) is a **separate** subsystem documented
  in [`src/ethan/telemetry/`](../src/ethan/telemetry/). It
...
- The leaderboard / contest opt-in (`OptInModal.tsx`) is a separate,
  ...
  the Ethan leaderboard. It is **not** the same as analytics and
