...
!!! note "Backward compatibility"
    The canonical locations are now `ethan.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `ethan.core.types` (for `RoutingContext`). The old `ethan.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `ethan.learning._stubs`.
...
```python
from ethan.learning.router import HeuristicRouter, build_routing_context
...
```python
from ethan.learning.router import build_routing_context
...
```python
from ethan.learning.trace_policy import TraceDrivenPolicy
...
```python
from ethan.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from ethan.learning.sft_policy import SFTPolicy
```
...
```python
from ethan.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from ethan.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from ethan.learning.heuristic_reward import HeuristicRewardFunction
...
over Ethan's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `ethan-learning`
