...
The Engine primitive provides the **inference runtime** -- the layer that connects Ethan to language model servers. All backends implement a uniform interface, making it straightforward to swap between local and cloud inference without changing application code.
...
Ethan automatically detects system hardware to recommend the best engine. Detection runs at config load time via `detect_hardware()`:
...
```python
from ethan.engine import discover_engines
...
Engine hosts and defaults are configured in `~/.ethan/config.toml` using **nested per-engine sub-sections**:
...
```python
from ethan.engine._base import messages_to_dicts
...
```python
from ethan.engine import EngineConnectionError
