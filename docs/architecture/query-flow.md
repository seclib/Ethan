...
This page traces the end-to-end journey of a user query through the Ethan system, from the moment it enters the CLI or SDK to the final response and telemetry recording.
...
Ethan supports two query processing paths, selected by the `--agent` CLI flag or the `agent` parameter in the SDK.
...
```python
config = load_config()  # Reads ~/.ethan/config.toml
```
...
```python
from ethan.learning import ensure_registered
...
The `TelemetryStore` subscribes to `TELEMETRY_RECORD` events on the EventBus and writes records to `~/.ethan/telemetry.db`.
