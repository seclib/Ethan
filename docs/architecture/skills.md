...
2. **Tolerant pass** — maps non-spec vendor fields to canonical locations via `FIELD_MAPPING` table. Unmapped fields are logged and preserved in `metadata.ethan.original_frontmatter`
...
Sidecar storage for optimization output at `~/.ethan/learning/skills/<name>/optimized.toml`:
