...
```python
from ethan.agents.orchestrator import OrchestratorAgent
...
!!! note "Backward compatibility"
    The old `from ethan.agents.react import ReActAgent` import path still works via a backward-compat shim. The registry alias `"react"` also maps to `NativeReActAgent`.

```python
from ethan.agents.native_react import NativeReActAgent
...
```python
from ethan.agents.native_openhands import NativeOpenHandsAgent
...
```python
from ethan.agents.rlm import RLMAgent
...
```python
from ethan.agents.openhands import OpenHandsAgent
...
Wraps the `@anthropic-ai/claude-code` SDK via a bundled Node.js subprocess bridge. Unlike every other agent, inference is handled entirely by the Claude Agent SDK -- the Ethan inference engine is not used. This makes `ClaudeCodeAgent` a true external agent, similar in spirit to `OpenHandsAgent` but implemented via subprocess rather than an importable Python SDK.
...
1. On first call, copies the bundled `claude_code_runner/` to `~/.ethan/claude_code_runner/` and runs `npm install --production` if `node_modules` is absent
...
- `cleanup_orphans()` removes any stale containers labelled `ethan-sandbox=true`
