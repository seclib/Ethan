...
Ethan follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in Ethan is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
Ethan is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
Ethan **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
```bash
jarvis init --force
# Detects hardware, writes ~/.ethan/config.toml with:
# [engine]
...
Ethan provides a **clean Python API** through the `Jarvis` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`ethan[server]`, `ethan[memory-colbert]`, etc.)
...
The API server (`jarvis serve`) implements the **OpenAI chat completions API format**, making Ethan a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to Ethan:
...
Ethan requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of Ethan.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `ethan[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `ethan[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `ethan[memory-bm25]` | `rank-bm25` |
| API server | `ethan[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `ethan[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `ethan[inference-vllm]` | `vllm` |
| PDF ingestion | `ethan[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `ethan[channel-whatsapp-baileys]` | Node.js 22+ |
