...
- **Persistence:** Data persists across restarts in `~/.ethan/memory.db`
...
from ethan.tools.storage.faiss_backend import FAISSMemory
from ethan.tools.storage.hybrid import HybridMemory
...
!!! note "Backward compatibility"
    The old imports (e.g., `from ethan.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `ethan.tools.storage.*`.
...
```python
from ethan.tools.storage.chunking import chunk_text, ChunkConfig
...
```python
from ethan.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from ethan.tools.storage.context import inject_context, ContextConfig
...
The default backend is configured in `~/.ethan/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:
