...
def list_ports():
    ...
    for srv, info in registry.items():
        print(
            f"{info['service_name']:<20} {info['container']:<20} "
            f"{info['host_port']:<10} {info['container_port']:<15}"
        )
...
def generate_compose():
    ...
    for srv, info in registry.items():
        compose["services"][srv] = {"ports": [f"{info['host_port']}:{info['container_port']}"]}
...
def main():
    ...
    alloc_parser.add_argument(
        "--container-port", type=int, help="Internal container port", required=True
    )
