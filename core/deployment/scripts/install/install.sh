#!/usr/bin/env bash
# install.sh — Ethan curl-pipe-bash installer.
#
# Usage:
#   curl -fsSL https://ethan.github.io/Ethan/install.sh | bash
#
...
#   OPENJARVIS_REPO_URL    git repo URL (default: https://github.com/ethan/Ethan.git)
...
# uv/git tooling installs to Windows paths the rest of Ethan can't
...
Ethan runs on Windows via WSL2. Two paths:
  ...
  2. Desktop app — download the .exe from the Releases page:
     https://github.com/ethan/Ethan/releases

See the WSL2 install guide for the full walkthrough:
  https://ethan.github.io/Ethan/getting-started/wsl2/
EOF
        exit 1
        ;;
esac
...
Ethan installs to $HOME/.openjarvis, not /usr/local. Re-run as your
...
Two ways forward:
  ...
  2. Pre-authenticate sudo before piping (caches credentials for 5 min):
       sudo -v && curl -fsSL https://ethan.github.io/Ethan/install.sh | bash
EOF
        exit 1
    fi
    local sudo=""
...
OPENJARVIS_REPO_URL="${OPENJARVIS_REPO_URL:-https://github.com/ethan/Ethan.git}"
...
# Ethan team can see where users drop off during install.
...
ensure_path() {
    ...
    if grep -q "Ethan" "$rc" 2>/dev/null; then
...
step clone_repo         "Clone Ethan repo" clone_repo
step copy_scripts       "Copy install scripts"  copy_scripts
step create_venv        "Create venv"           create_venv
step editable_install   "Install Ethan"    editable_install
