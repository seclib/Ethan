...
OpenJarvis supports both development and production Docker deployments with a dedicated network, persistent volumes, and optional services.
...
- **Backend** (FastAPI) — multi-stage build, non-root user, healthcheck
- **Frontend** (Nginx) — static files with reverse proxy to backend
- **Ollama** (optional) — local LLM inference via `COMPOSE_PROFILES=ollama`
- **PostgreSQL** (optional) — database via `COMPOSE_PROFILES=postgres`
- **ChromaDB** (optional) — vector store via `COMPOSE_PROFILES=chromadb`
- **Redis** (optional) — cache & queue via `COMPOSE_PROFILES=redis`

Enable optional services with profiles:

```bash
COMPOSE_PROFILES=ollama,postgres,chromadb,redis docker compose -f deploy/docker/docker-compose.yml up -d
```
...
make dev
# or: docker compose -f deploy/docker/docker-compose.dev.yml up --build
```
...
| Volume | Purpose |
|--------|---------|
| `jarvis-config` | Configuration files |
| `jarvis-logs` | Application logs |
| `jarvis-workspace` | Workspace for file operations |
| `jarvis-memory` | Vector memory store |
| `jarvis-uploads` | Uploaded files |
| `jarvis-cache` | Cache data |
| `jarvis-models` | Local models |
| `ollama-models` | Ollama model storage |

### Architecture

See [docs/architecture.md](docs/architecture.md) for the complete architecture documentation, including component analysis, dependency graph, and improvement roadmap.
