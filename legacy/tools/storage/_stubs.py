...
#: constructed because the mandatory ``ethan_rust`` extension is missing
...
RUST_MISSING_HINT = (
    "Memory backend unavailable: the native `ethan_rust` extension is not "
    "installed in this environment. Build it into the venv that runs the server "
    "with `uv run maturin develop -m rust/crates/ethan-python/Cargo.toml` "
    "(needs rustc >= 1.88), then restart. Verify with "
    '`python -c "from ethan._rust_bridge import RUST_AVAILABLE; '
...
class MemoryBackendUnavailable(RuntimeError):
    ...
    ``ethan_rust`` extension is missing from the current environment.
