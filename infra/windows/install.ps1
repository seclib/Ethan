<#
.SYNOPSIS
    Ethan native Windows installer.
...
.DESCRIPTION
    ...
    Steps:
      ...
      5. Clone the Ethan repository to $env:LOCALAPPDATA\Ethan
    ...
    Usage (one-liner):
      irm https://ethan.github.io/Ethan/install.ps1 | iex

    Usage (file invocation, supports flags):
      irm https://ethan.github.io/Ethan/install.ps1 -OutFile install.ps1
...
if ($pyMajor -ne 3 -or $pyMinor -lt 10 -or $pyMinor -gt 13) {
    Write-Fail @"
Found Python $pyMajor.$pyMinor at $pythonExe, but Ethan requires
...
$installRoot = if ($env:OPENJARVIS_HOME) {
    ...
    Join-Path $env:LOCALAPPDATA 'Ethan'
}
...
$repoUrl = if ($env:OPENJARVIS_REPO_URL) {
    ...
    'https://github.com/ethan/Ethan.git'
}
...
# so the shim is self-locating - moving %LOCALAPPDATA%\Ethan won't
...
# Add %LOCALAPPDATA%\Ethan\bin to User PATH if it isn't already
...
# the entry as `%LOCALAPPDATA%\Ethan\bin` (unexpanded) into User
...
if ($Service) {
    ...
    if ($isInteractive) {
        $reply = Read-Host "Register Ethan as a Windows scheduled task (auto-start at logon, loopback only)? [y/N]"
...
if ($shouldInstallService) {
    ...
    Write-Ok "Scheduled task 'Ethan' registered (loopback default)."
}
...
Write-Host "  |   Ethan install complete    |" -ForegroundColor Green
Write-Host "  +----------------------------------+" -ForegroundColor Green
Write-Host ""
...
if ($shouldInstallService) {
    Write-Host ""
    Write-Host "  Service: schtasks /Query /TN Ethan     (status)"
    Write-Host "           powershell -File `"$serviceScript`" uninstall    (remove)"
}
Write-Host ""
Write-Host "  Docs:    https://ethan.github.io/Ethan/"
Write-Host ""
