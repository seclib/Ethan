...
5. Clones the Ethan repository to `%LOCALAPPDATA%\Ethan`
...
```powershell
$srv = "$env:LOCALAPPDATA\Ethan\src\deploy\windows\jarvis-service.ps1"
...
```powershell
powershell -ExecutionPolicy Bypass -File "$env:LOCALAPPDATA\Ethan\src\deploy\windows\jarvis-service.ps1" uninstall
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\Ethan"
```
