<#
.SYNOPSIS
    Register / unregister the Ethan Windows scheduled task.

.DESCRIPTION
    The Windows equivalent of deploy/systemd/openjarvis.service and
    deploy/launchd/com.openjarvis.plist.

    Registers a per-user scheduled task named "Ethan" that starts
    ...
    Arguments (install only):
      -InstallRoot <path>  default: %LOCALAPPDATA%\Ethan (matches
...
function Get-DefaultInstallRoot {
    ...
    return (Join-Path $env:LOCALAPPDATA 'Ethan')
}
...
function Install-Task {
    ...
    if (-not (Test-Path $srcDir)) {
        Write-Fail "Ethan source not found at $srcDir. Run install.ps1 first."
    }
...
"@
    ...
    Register-ScheduledTask `
        ...
        -Settings $settings `
        -Principal $principal `
        -Description 'Ethan API server (loopback default — see deploy/windows/README.md)' | Out-Null
