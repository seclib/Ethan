...
This tutorial demonstrates how to use Ethan to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.
...
- **Model selection**: Use `--model` to target a specific model, or let Ethan auto-select from what is available.
