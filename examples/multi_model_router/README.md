...
Route queries to the cheapest capable model using Ethan's learning/routing
system. Simple queries go to small fast models; complex code or math queries go
to larger models.

## Requirements

- Ethan installed (`git clone https://github.com/ethan/Ethan.git && cd Ethan && uv sync` or `uv sync --extra dev`)
...
The script uses Ethan's routing infrastructure from the learning pillar:
