/**
 * deep-link.ts — Handler for ethan:// deep links in the Tauri desktop app.
 *
 ...
 *   ethan://research/{session_id}   → open a research session
 *   ethan://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `ethan://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `ethan://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `ethan:` scheme.
 *
 * @example
 * parseDeepLink("ethan://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("ethan://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "ethan:") return null;

    // The URL constructor treats "ethan://research/abc123" such that
