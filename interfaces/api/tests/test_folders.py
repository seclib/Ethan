...
def test_tree_move_and_hierarchy_routes(folders):
    ...
    root = asyncio.run(folders_mod().create_folder({"name": "OSINT", "user_id": "alice"}))
    child = asyncio.run(
        folders_mod().create_folder({"name": "Recon", "parent_id": root["id"], "user_id": "alice"})
    )
...
def test_resource_classification_routes(folders):
    ...
    content = asyncio.run(folders_mod().list_folder_resources(osint["id"], resource_type=None))
    ...
    folders_of = asyncio.run(folders_mod().list_folders_of_resource("collection", collection["id"]))
    assert {f["name"] for f in folders_of} == {"OSINT", "Code"}

    result = asyncio.run(
        folders_mod().move_resource_between_folders(
            {
    ...
    untagged = asyncio.run(folders_mod().list_untagged_resources(resource_type="collection"))
    ...
    with pytest.raises(HTTPException) as exc:
        asyncio.run(folders_mod().detach_resource(code["id"], "collection", collection["id"]))
    assert exc.value.status_code == 404
...
def test_folder_index_route(folders):
    ...
    index = asyncio.run(folders_mod().get_folder_index(resource_type="collection"))
