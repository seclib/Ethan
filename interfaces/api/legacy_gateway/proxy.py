...
async def proxy_ollama(path: str, request: Request):
    ...
    normalized_path = (
        path if path.startswith("/api/") or path.startswith("/v1/") else f"/api/{path}"
    )

    if normalized_path not in ALLOWED_PATHS:
        raise HTTPException(status_code=403, detail=f"Path '{normalized_path}' is not allowed")
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail="Unable to reach Ollama service")
    except httpx.TimeoutException:
        raise HTTPException(status_code=504, detail="Ollama service timed out")
