...
# Import proxy router
from interfaces.api.legacy_gateway.proxy import router as proxy_router
from nats.aio.client import Client as NATSClient
from pydantic import BaseModel

app = FastAPI(title="ETHAN API", description="Cognitive Runtime API Gateway", version="1.0.0")
...
async def startup():
    ...
    for attempt in range(1, 11):
        try:
            ...
            nats_client = await asyncio.wait_for(nats.connect(nats_url), timeout=min(10, remaining))
...
async def root():
    """Root endpoint"""
    return {"name": "ETHAN API", "version": "1.0.0", "status": "running"}
...
async def send_message(request: MessageRequest):
    ...
    event = {
        ...
        "payload": {"content": request.content, "model": request.model, "stream": request.stream},
    }

    # Publish to NATS
    await nats_client.publish("ethan.interface.api", json.dumps(event).encode())
    ...
    return MessageResponse(
        ...
        metadata={"model": request.model, "tokens": len(request.content.split())},
    )
...
async def get_status():
    ...
    return StatusResponse(
        runtime={"state": "RUNNING", "uptime": "2h 15m", "pid": 12345},
        services=[
...
async def start_services(request: ServiceRequest):
    ...
    # Publish to NATS
    await nats_client.publish("ethan.runtime.command", json.dumps(event).encode())

    return {"success": True, "services": request.services}
