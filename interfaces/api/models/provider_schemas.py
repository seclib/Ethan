...
class ProviderCreate(BaseModel):
    ...
    type: str = Field(
        ...,
        description=(
            "Type de provider: ollama, openai, anthropic, vllm, llamacpp, "
            "lmstudio, gemini, openai-compatible, custom"
        ),
    )
    ...
    options: dict[str, Any] = Field(
        default_factory=dict, description="Options spécifiques au provider"
    )
...
class ProviderResponse(BaseModel):
    ...
    status: str = Field(
        "unknown", description="État de connexion: connected, disconnected, error, unknown"
    )
    default_model: str = Field("", description="Modèle par défaut")
    ...
    auth_methods: list[str] = Field(
        default_factory=list, description="Méthodes d'authentification supportées"
    )
    # Booléen uniquement — la clé API elle-même n'est JAMAIS sérialisée.
    has_api_key: bool = Field(
        False, description="Une clé/token est configuré (secrets jamais exposés)"
    )
