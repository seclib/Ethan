...
class CommandRegistry:
    ...
    def list_commands(self, group=None):
        ...
        for cmd in self._commands.values():
            # `group` accepte un nom exact ("core") ou un préfixe de namespace
            # ("plugin" / "plugin:"). `rstrip(":")` évite le double deux-points
            # qui rendait `list_commands("plugin:")` systématiquement vide.
            matches = (
                group is None
                or cmd.group == group
                or (isinstance(group, str) and cmd.group.startswith(group.rstrip(":") + ":"))
            )
            if matches:
                cmds.append(cmd)
        return cmds
...
registry.register(
    Command(
        "chat", "core", "Interactive AI chat", "ethan chat", ["ethan chat", "ethan chat --resume"]
    )
)
...
registry.register(
    Command(
        ...
        "Tail system logs",
        "ethan logs",
        ["ethan logs --follow", "ethan logs --lines 50"],
    )
)
...
registry.register(
    Command("daemon", "advanced", "Control background daemon", "ethan daemon <start|stop|restart>")
)
