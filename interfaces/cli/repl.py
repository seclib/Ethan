...
from interfaces.cli.core.triggers import check_triggers
...
def handle_command(
    input_str: str, session: Session, client: RuntimeClient, session_manager: SessionManager
):
...
def handle_message(
    content: str, session: Session, client: RuntimeClient, session_manager: SessionManager
):
    """Handle user message."""
    ...
    # Send to Runtime
    request = {"type": "message.send", "session_id": session.id, "content": content, "stream": True}
    ...
    try:
        ...
        session.add_message(
            "assistant", full_response, metadata=metadata if chunk.get("done") else {}
        )
