...
Error: strict mode violation: locator('aside') resolved to 2 elements:
    1) <aside class="fixed right-0 top-0 z-[60] h-[100dvh] w-full border-l bg-background shadow-2xl transition-transform duration-300 md:w-[400px] translate-x-full">…</aside> aka getByText('InspectorSelect an item (')
    2) <aside class="hidden lg:flex flex-col w-[240px] border-l border-white/5 bg-white/[0.015]">…</aside> aka getByRole('complementary').filter({ hasText: 'NETWORKONLINEAI' })
